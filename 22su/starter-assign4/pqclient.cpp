...
Vector<DataPoint> topK(istream& stream, int k) {
    PQArray topKpq;
    Vector<DataPoint> topKElems;

    int streamSize = handleStream(stream, topKpq, k);
    ...
    // // 初始化vec
    fillVector(topKElems, k);

    for (int i = k - 1; i >= 0; i--) {
        topKElems[i] = topKpq.dequeue();
    }

    return topKElems;
}

void addNewElem (PQArray& pq, DataPoint newElem, int k) {
    if (pq.size() == k) {
        double smallest = pq.peek().priority;
        // 前k个已经满了
        if (smallest < newElem.priority) {
            pq.dequeue();
            pq.enqueue(newElem);
        }
    }
...
/* 返回stream大小
 */
int handleStream(istream& stream, PQArray& pq, int k) {
    DataPoint cur;
    int cnt = 0;
    // 每次取出1个DataPoint类型的cur进行处理
    while (stream >> cur) {
        addNewElem(pq, cur, k);
        cnt += 1;
    }
...
//         while(stream>>x){
//             if(topKpq.size() >= k){
//                 DataPoint end = topKpq.dequeue();
//                 if(x.priority>end.priority){
//                     topKpq.enqueue(x);
//                 }else{
//                     topKpq.enqueue(end);
//                 }
...
//             k = topKpq.size();
//         }
//         Vector<DataPoint> topKElems(k,{"",0});
//         for(int i =k-1;i>-1;i--){
//             topKElems[i] = topKpq.dequeue();
//         }
//         return topKElems;
